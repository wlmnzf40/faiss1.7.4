...
// Processes 8 float values from 1 coarse centroid (8 floats) + 4 fine centroids (2 floats each).
// Returns {
...
inline float32x4x2_t elementaryBlock2x4b(
    ...
    // load coarse (8 consecutive floats)
    const auto coarseValue0 = vld1q_f32(coarse);
    const auto coarseValue1 = vld1q_f32(coarse + 4);
    // load fine pairs (FINE_SIZE=2: 64-bit loads, then combine)
    const auto v0 = vld1_f32(fine0);
    const auto v1 = vld1_f32(fine1);
    const auto v2 = vld1_f32(fine2);
    const auto v3 = vld1_f32(fine3);
    const auto fineValue0 = vcombine_f32(v0, v1);
    const auto fineValue1 = vcombine_f32(v2, v3);
    // add coarse and fine
    return {vaddq_f32(coarseValue0, fineValue0), vaddq_f32(coarseValue1, fineValue1)};
}

// Processes 8 float values.
// Returns {
//   [0..1] = existingValue[0..1] + weight * (*coarse[0..1] + *fine0[0..1]);
//   [2..3] = existingValue[2..3] + weight * (*coarse[2..3] + *fine1[0..1]);
//   [4..5] = existingValue[4..5] + weight * (*coarse[4..5] + *fine2[0..1]);
//   [6..7] = existingValue[6..7] + weight * (*coarse[6..7] + *fine3[0..1]);
// }
...
// NEON-optimized specialization for FINE_SIZE=2.
// Processes 8 floats per loop (1 coarse * 8 + 4 fine centroids * 2), advances CPOS+8.
template <
        intptr_t DIM,
        intptr_t COARSE_SIZE,
        intptr_t COARSE_BITS,
...
struct Index2LevelDecoderImpl<
    ...
    static void store(
        ...
        // process chunks, 2 float, but 8 floats per loop
    ...
    static void accum(
        ...
        auto existingValue = float32x4x2_t{
                vld1q_f32(outputAccum + CPOS), vld1q_f32(outputAccum + CPOS + 4)};
        ...
        Index2LevelDecoderImpl<DIM, COARSE_SIZE, FINE_SIZE, COARSE_BITS, FINE_BITS, CPOS + 8>::accum(
                pqCoarseCentroids0, pqFineCentroids0, code0, weight0, outputAccum);
    }
    ...
    static void accum(
        ...
        Index2LevelDecoderImpl<DIM, COARSE_SIZE, FINE_SIZE, COARSE_BITS, FINE_BITS, CPOS + 8>::accum(
                ...
                code0, weight0, code1, weight1, outputAccum);
    }
    ...
    static void accum(
        ...
        Index2LevelDecoderImpl<DIM, COARSE_SIZE, FINE_SIZE, COARSE_BITS, FINE_BITS, CPOS + 8>::accum(
                pqCoarseCentroids, pqFineCentroids,
                code0, weight0, code1, weight1, code2, weight2, outputAccum);
    }
};
...
struct Index2LevelDecoderImpl<
    ...
    static void store(
        ...
        const intptr_t fineCode0 =
                detail::UintReader<DIM, FINE_SIZE, FINE_BITS, fineCentroidIdx>::
                        get(fine0);
        const float* const csrc = pqCoarseCentroids0 +
                (coarseCentroidIdx * COARSE_TABLE_BYTES + coarseCode0) *
                        COARSE_SIZE +
                coarseCentroidOffset;
        const float* const fsrc = pqFineCentroids0 +
                (fineCentroidIdx * FINE_TABLE_BYTES + fineCode0) * FINE_SIZE +
                fineCentroidOffset;
        for (intptr_t j = 0; j < QPOS_LEFT; j++) {
            outputStore[CPOS + j] = csrc[j] + fsrc[j];
        }
        Index2LevelDecoderImpl<
                DIM, COARSE_SIZE, FINE_SIZE, COARSE_BITS, FINE_BITS,
                CPOS + QPOS_LEFT>::
                store(pqCoarseCentroids0, pqFineCentroids0, code0, outputStore);
    }
    ...
    static void accum(
        ...
        const float* const fsrc = pqFineCentroids0 +
                ...
                fineCentroidOffset;
        for (intptr_t j = 0; j < QPOS_LEFT; j++) {
            outputAccum[CPOS + j] += weight0 * (csrc[j] + fsrc[j]);
        }
        Index2LevelDecoderImpl<
                DIM, COARSE_SIZE, FINE_SIZE, COARSE_BITS, FINE_BITS,
                CPOS + QPOS_LEFT>::
    ...
    static void accum(
        ...
        const intptr_t fineCode1 =
                detail::UintReader<DIM, FINE_SIZE, FINE_BITS, fineCentroidIdx>::
                        get(fine1);
        const float* const csrc0 = pqCoarseCentroids0 +
                (coarseCentroidIdx * COARSE_TABLE_BYTES + coarseCode0) *
                        COARSE_SIZE +
                coarseCentroidOffset;
        const float* const fsrc0 = pqFineCentroids0 +
                (fineCentroidIdx * FINE_TABLE_BYTES + fineCode0) * FINE_SIZE +
                fineCentroidOffset;
        const float* const csrc1 = pqCoarseCentroids1 +
                (coarseCentroidIdx * COARSE_TABLE_BYTES + coarseCode1) *
                        COARSE_SIZE +
                coarseCentroidOffset;
        const float* const fsrc1 = pqFineCentroids1 +
                (fineCentroidIdx * FINE_TABLE_BYTES + fineCode1) * FINE_SIZE +
                fineCentroidOffset;
        for (intptr_t j = 0; j < QPOS_LEFT; j++) {
            outputAccum[CPOS + j] +=
                    weight0 * (csrc0[j] + fsrc0[j]) +
                    weight1 * (csrc1[j] + fsrc1[j]);
        }
        Index2LevelDecoderImpl<
                DIM, COARSE_SIZE, FINE_SIZE, COARSE_BITS, FINE_BITS,
                CPOS + QPOS_LEFT>::
    ...
    static void accum(
        ...
        const intptr_t fineCode1 =
                detail::UintReader<DIM, FINE_SIZE, FINE_BITS, fineCentroidIdx>::
                        get(fine1);
        const float* const csrc0 = pqCoarseCentroids +
                (coarseCentroidIdx * COARSE_TABLE_BYTES + coarseCode0) *
                        COARSE_SIZE +
                coarseCentroidOffset;
        const float* const fsrc0 = pqFineCentroids +
                (fineCentroidIdx * FINE_TABLE_BYTES + fineCode0) * FINE_SIZE +
                fineCentroidOffset;
        const float* const csrc1 = pqCoarseCentroids +
                (coarseCentroidIdx * COARSE_TABLE_BYTES + coarseCode1) *
                        COARSE_SIZE +
                coarseCentroidOffset;
        const float* const fsrc1 = pqFineCentroids +
                (fineCentroidIdx * FINE_TABLE_BYTES + fineCode1) * FINE_SIZE +
                fineCentroidOffset;
        for (intptr_t j = 0; j < QPOS_LEFT; j++) {
            outputAccum[CPOS + j] +=
    ...
    static void accum(
        ...
        const float* const fsrc1 = pqFineCentroids1 +
                (fineCentroidIdx * FINE_TABLE_BYTES + fineCode1) * FINE_SIZE +
                fineCentroidOffset;
        const float* const csrc2 = pqCoarseCentroids2 +
                (coarseCentroidIdx * COARSE_TABLE_BYTES + coarseCode2) *
                        COARSE_SIZE +
                coarseCentroidOffset;
        const float* const fsrc2 = pqFineCentroids2 +
    ...
    static void accum(
        ...
        const float* const fsrc1 = pqFineCentroids +
                (fineCentroidIdx * FINE_TABLE_BYTES + fineCode1) * FINE_SIZE +
                fineCentroidOffset;
        const float* const csrc2 = pqCoarseCentroids +
                (coarseCentroidIdx * COARSE_TABLE_BYTES + coarseCode2) *
                        COARSE_SIZE +
                coarseCentroidOffset;
        const float* const fsrc2 = pqFineCentroids +
                (fineCentroidIdx * FINE_TABLE_BYTES + fineCode2) * FINE_SIZE +
                fineCentroidOffset;
        for (intptr_t j = 0; j < QPOS_LEFT; j++) {
            outputAccum[CPOS + j] +=
